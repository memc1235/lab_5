...
function loadStatsPage(){
	...
	for(row_count = 2; row_count < table.rows.length; row_count++){
		buffScore = table.rows[row_count].cells[2].innerHTML;
		oppScore = table.rows[row_count].cells[3].innerHTML;
		diff = buffScore - oppScore;
		if(diff > 0){
			buffWin++;
			table.rows[row_count].cells[4].innerHTML = "Colorado";
		}
		else{
			buffLoss++;
...
function loadPlayersPage(){
	...
	for (let i = 0; i < size; i++){
		...
		select.innerHTML += "<a class=\"dropdown-item\" href=\"#\" onclick=\"switchPlayers("+i+")\">" + name + "</a>";
	}
